...
const newGroupChat=TryCatch(async (req,res,next)=>{
 const {name,members}=req.body
    ...
    //we will alert all the members
    emitEvent(req,ALERT,allMembers,`Welcome to ${name} group`)
    //we will refetch chat of the group
...
const getMyChats=TryCatch(async (req,res,next)=>{
   ...
   const addMembers=TryCatch(async(req,res,next)=>{
    ...
    if(chat.members.length>100){
        return next(new ErrorHandler("Group can have maximum 100 members",400))
    }
...
const removeMember=TryCatch(async(req,res,next)=>{
    ...
    if(!chat.members.includes(userId)){
        return next(new ErrorHandler("This user is not a member of this group",400))
    }
    const allChatMembers=chat.members.map((i)=>i.toString())
    chat.members=chat.members.filter((member)=>member.toString()!==userId.toString())
    await chat.save()
    console.log(userThatWillBeRemoved)
    emitEvent(req,ALERT,chat.members,`${userThatWillBeRemoved.name} have been removed from ${chat.name} group`)
...
const renameGroup=TryCatch(async (req,res,next)=>{
    ...
    if(chat.creator.toString()!==req.user.toString()){
        return next(new ErrorHandler("You arent allowed to rename the group",403))
    }
    chat.name=name;
    await chat.save()
    emitEvent(req,REFETCH_CHATS,chat.members)
...
const deleteChat=TryCatch(async (req,res,next)=>{
    ...
    if(chat.groupChat && chat.creator.toString()!==req.user.toString() ){
        return next(new ErrorHandler("You arent allowed to delete the group",403))
    }
    if(chat.groupChat && !members.includes(req.user.toString())){
        return next(new ErrorHandler("You arent allowed to delete the group",403))
    }
    //here we have to delete all the messages as well as attachment or files from cloudinary
    const messagesWithAttachments=await Message.find({chat:chatId,
        attachments:{$exists:true,$ne:[]}
    })
    const public_ids=[]
    //public id of all attachments
...
const getMessages=TryCatch(async (req,res,next)=>{
    ...
    if(!chat.members.includes(req.user.toString())){
        return next(new ErrorHandler("You arent Allowed to access this chat"),403)
    }
